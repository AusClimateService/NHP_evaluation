...
ref_start_year=$2
ref_end_year=$3
path_climate_ref=$4 # where is the input data stored?
out_path_ref=$5
timescale=$6
statistic=$7
...
if [ -f ${fn_merged} ]; then
        ...
        if [ -f ${fn_year} ]; then
            ...
            if [ ${timescale} == 'seas' ]; then
                ...
                let year_before=${year}-1
                    ...
                    # test if the file for the year before exists (not the case for solar in 1989)
                    if [ -f ${input_path}/${var}*${year_before}.nc ]; then
                        ...
                        echo "##### No data for year ${year_before}. Skipping."
                        continue
                    fi
            fi
    ...
    # if the calculation is for solar radiation (seasonal) and the start year is 1990, then the first year was skipped
    if [ ${var} == 'solar_exposure_day' ] & [ ${timescale} == 'seas' ] & [ ${ref_start_year} == '1990' ]; then
        let "n_years = ${ref_end_year} - ${ref_start_year}"
    fi
